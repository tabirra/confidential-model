...
Supports two mutually exclusive ways to obtain the AES-256-GCM decryption
key:

1. **Layer 1 (default)**: read it from a Kubernetes Secret mounted as a
   file (see k8s/consumer-pod.yaml). The host/kubelet controls delivery;
   the consumer trusts whoever populated the Secret.
2. **Layer 3 (attested key release)**: fetch it from Trustee KBS through
...
Either way, the rest of the pipeline is unchanged:

1. Obtain the decryption key (Secret file, or attested KBS/CDH fetch).
2. Download the encrypted artifact + manifest from the Hugging Face Hub.
3. Verify the ciphertext checksum against the manifest.
4. Decrypt the archive with AES-256-GCM.
5. Extract it and load the model with `transformers`.
6. Run a small sanity inference to prove the model is usable.
